...
// //Versiones acceptadas por el programa
// const ALIAS_COLUMNAS: Record<"codigo" | "descripcion" | "unidad" | "cantidad" | "valorUnitarioOficial", string[]> = {
//   codigo: ["codigo", "código", "item", "ítem", "cod"],
//   descripcion: ["descripcion", "descripción", "actividad", "concepto", "detalle"],
//   unidad: ["unidad", "um", "un", "u","unidad de medida"],
//   cantidad: ["cantidad", "cant", "cant."],
...
// function parsearValorUnitarioOficial(valor: unknown): number | null {
//   if (typeof valor === "number") return valor
//   const texto = String(valor ?? "").trim()
//   if (texto === "") return null
//   const soloNumero = texto.replace(/[^0-9.,-]/g, "")
...
//     // "no sabemos el original").
//     //const valorTotalRaw = colMap.valorTotal ? fila[colMap.valorTotal] : null
//     const valorUnitarioOficialRaw = colMap.valorUnitarioOficial ? fila[colMap.valorUnitarioOficial] : null

//     // const precioOriginal = parsearValorTotalOriginal(valorTotalRaw)
//     const precioOriginal = parsearValorUnitarioOficial(valorUnitarioOficialRaw)
...
//       {presupuesto.length > 0 && (
//         <aside className="sticky top-16 h-[calc(100vh-4rem)] w-72 shrink-0 overflow-y-auto border-r p-4">
//           <PresupuestoTree
//             data={presupuesto}
//             onSeleccionar={handleSeleccionarEnArbol}
//             idSeleccionado={itemResaltadoId}
...
// la hoja 2 "APU" si el archivo la trae -- si no la trae, bloques queda
// vacío y todo el resto del flujo (guardar, etc.) se comporta EXACTAMENTE
// igual que antes de este cambio. La diferencia con antes: ahora se
// distingue "no tiene hoja APU" (aviso, no bloqueante -- puede ser a
// propósito) de "el archivo no se pudo leer" (error, bloqueante -- algo
// realmente falló). Antes ambos casos (y el de "no tiene hoja APU")
// quedaban en silencio total del lado del aviso: la tabla de pendientes
// salía vacía sin ninguna pista de qué pasó.
async function procesarPresupuesto(
  documentoPresupuesto: File
): Promise<{ items: ItemPresupuesto[]; bloquesApu: BloqueApu[]; avisoApu: string | null }> {
  let buffer: ArrayBuffer
  try {
    buffer = await documentoPresupuesto.arrayBuffer()
  } catch {
    throw new Error("No se pudo leer el archivo -- ¿está corrupto o se interrumpió la subida?")
  }
  ...
  try {
    ...
    throw new Error(
      "Formato no reconocido -- el archivo no parece ser un Excel válido (.xlsx/.xls). " +
        "¿Seguro que es el archivo correcto?"
    )
  }

  if (workbook.SheetNames.length === 0) {
    throw new Error("El archivo no tiene ninguna hoja -- revisa que sea el Excel correcto.")
  }
  ...
  if (!nombreHojaApu) {
    return {
      ...
      bloquesApu: [],
      avisoApu:
        `No se encontró página de APUs -- ninguna hoja de este archivo se llama "APU" ` +
        `(hojas encontradas: ${workbook.SheetNames.join(", ")}). Si el Excel debía traer ` +
        `análisis de precios, revisa que la pestaña se llame exactamente "APU". Si este ` +
        `presupuesto no trae APU a propósito, puedes ignorar este aviso.`,
    }
  }
  ...
  return { items, bloquesApu: bloques, avisoApu: null }
}
...
export default function Presupuestos() {
  ...
  const [guardando, setGuardando] = useState(false)
  const [error, setError] = useState<string | null>(null)
  // Aviso NO bloqueante para problemas de lectura del Excel que no
  // deberían impedir seguir (ej. no encontró la hoja "APU" -- puede ser
  // a propósito, un presupuesto sin análisis de precios es válido). Se
  // muestra en ámbar, separado de `error` (rojo, bloqueante). Antes
  // este caso quedaba en silencio total: la tabla de pendientes salía
  // vacía sin ninguna pista de por qué.
  const [avisoImportApu, setAvisoImportApu] = useState<string | null>(null)
  const [itemApuAbierto, setItemApuAbierto] = useState<ItemPresupuesto | null>(null)
  const [itemResaltadoId, setItemResaltadoId] = useState<string | null>(null)
  const [dialogoAgregarAbierto, setDialogoAgregarAbierto] = useState(false)
  const [exportando, setExportando] = useState(false)
  ...
  const [itemIdsParaRevisar, setItemIdsParaRevisar] = useState<string[]>([])
  const [modoGeneralRevision, setModoGeneralRevision] = useState(false)
  // Ítems con al menos una línea de transporte sin precio -- separado de
  // estadosApu (que ya los cuenta como "pendiente" para el color de la
  // fila) porque el banner y el botón de "Revisar" de transporte son
  // específicos: hay que poder abrir el diálogo directo en la pestaña
  // "Transporte", no en la de Insumos.
  const [itemIdsTransportePendiente, setItemIdsTransportePendiente] = useState<string[]>([])
  // Qué pestaña debe abrir el diálogo la próxima vez -- por defecto
  // Insumos (mismo comportamiento de siempre), pero el banner de
  // transporte lo cambia a "transporte" antes de abrir.
  const [tabInicialRevision, setTabInicialRevision] = useState<"insumos" | "mano_obra" | "equipo" | "transporte">(
    "insumos"
  )
  ...
  async function refrescarEstadosApu(idsAConsultar: string[]) {
    ...
    try {
      ...
      // Mismo criterio de "actualizar en el lugar" que arriba -- se
      // reconstruye solo la porción correspondiente a idsAConsultar, sin
      // tocar el resto de la lista (que puede venir de otra tanda).
      setItemIdsTransportePendiente((prev) => {
        const consultados = new Set(idsAConsultar)
        const conservados = prev.filter((id) => !consultados.has(id))
        return [...conservados, ...idsTransporte]
      })
    } catch (e) {
      console.error("No se pudo cargar el estado de APU de los ítems:", e)
    }
  ...
  // Banner de transporte -- abre el mismo diálogo, pero directo en la
  // pestaña "Transporte" y acotado a los ítems que de verdad tienen algo
  // pendiente de transporte (no todos los que tienen CUALQUIER pendiente).
  function handleAbrirRevisionTransporte() {
    setModoGeneralRevision(false)
    setItemIdsParaRevisar(itemIdsTransportePendiente)
    setTabInicialRevision("transporte")
    setDialogoRevisionAbierto(true)
  }
  ...
  async function guardarPresupuestoConApu(
    ...
    try {
      ...
      // "Volqueta" ya no se excluye del import -- se reclasifica a
      // TRANSPORTE del lado del servidor (ver reclasificarVolquetaComoTransporte
      // en actions.ts) y cae sola en el banner de "N pendientes de
      // transporte" que ya existe (reactivo a estadosApu, igual que
      // cualquier otra línea de transporte) -- no necesita un aviso
      // aparte acá.
    } catch (e) {
      setError(e instanceof Error ? e.message : "No se pudo guardar el APU importado.")
      aplicarItemsAlPresupuesto() // por si falló antes de aplicarlos (ej. handleGuardar) -- no se pierde lo que se alcanzó a parsear. Si ya se habían aplicado, llamar de nuevo no hace daño.
  ...
  async function handleFileSelected(file: File) {
    setError(null)
    setAvisoImportApu(null)

    let resultado: { items: ItemPresupuesto[]; bloquesApu: BloqueApu[]; avisoApu: string | null }
    try {
      resultado = await procesarPresupuesto(file)
    } catch (e) {
      // Incluye tanto ErrorFilasInvalidas (algún código no se pudo
    ...
    if (resultado.avisoApu) setAvisoImportApu(resultado.avisoApu)
        ...
        <div className="space-y-2">
          ...
          {avisoImportApu && (
            <div className="flex items-start justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              <span>{avisoImportApu}</span>
              <button
                type="button"
                onClick={() => setAvisoImportApu(null)}
          ...
          {/* También se muestra acá (además de junto al botón de
              guardar, más abajo) porque esa otra ubicación vive dentro
              de {presupuesto.length > 0 && (...)} -- si el archivo
              falla tan mal que ni siquiera se alcanzan a sacar los
              ítems de la hoja 1 (ej. "Formato no reconocido"),
              `presupuesto` queda vacío y ese otro bloque ni se
              renderiza. Sin esto, ese caso quedaba completamente mudo:
              ni tabla, ni error, nada. */}
          {error && presupuesto.length === 0 && (
            <p className="whitespace-pre-line text-sm text-destructive">{error}</p>
          )}
        </div>
        ...
        {itemIdsTransportePendiente.length > 0 && !dialogoRevisionAbierto && (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-sky-300 bg-sky-50 px-4 py-3">
            <p className="text-sm text-sky-900">
              {itemIdsTransportePendiente.length} ítem(s) necesitan precio de transporte -- descarga
              el Excel, complétalo y súbelo de vuelta.
            </p>
            <Button size="sm" onClick={handleAbrirRevisionTransporte}>
              Revisar pendientes
            </Button>
          </div>
        )}
