...
// // ---------------------------------------------------------------------------
// // Lectura del Excel: ya NO se lee una columna "Nivel" -- el nivel se
// // calcula del propio código con calcularNivelDesdeCodigo (ver
// // lib/calcular-nivel.ts). Acepta dos formatos de código:
// //   - con puntos:      1 / 1.1 / 4.1.1.1
// //   - dígitos seguidos: 1 / 101 / 10101
// //
// // Las columnas que debe traer el Excel son Código, Descripción, Unidad
// // y Cantidad (esta última opcional -- si falta, queda en 0). Se
// // reconocen por alias, no por el nombre exacto del header.
// // ---------------------------------------------------------------------------

// const ALIAS_COLUMNAS: Record<"codigo" | "descripcion" | "unidad" | "cantidad", string[]> = {
...
// // Convierte un valor de celda de cantidad a número, aceptando tanto un
// // number nativo (celda con formato numérico en Excel -- lo más común,
// // XLSX ya lo entrega parseado) como un string en formato colombiano
// // "11.720,00" (celda con formato de texto) -- ahí el punto es separador
// // de miles y la coma es el decimal, al revés de Number() de JS.
...
// async function procesarPresupuesto(documentoPresupuesto: File): Promise<ItemPresupuesto[]> {
//   const buffer = await documentoPresupuesto.arrayBuffer()
//   const workbook = XLSX.read(buffer)
//   const sheetName = workbook.SheetNames[0]
//   const sheet = workbook.Sheets[sheetName]
//   const data = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet)
//   return procesarOrdenPresupuesto(data)
// }
...
//   // Antes: presupuestosExistentes (lista), cargandoExistentes,
//   // avisoDescartado, presupuestoDestinoElegido, itemsPendientesDeOrigen.
//   // Ahora un proyecto tiene, como mucho, UN presupuesto (constraint
//   // presupuestos_proyecto_id_unique) -- no hace falta lista ni elegir
//   // "cuál": o existe o no existe.
...
//   // Versiones (Fase 4): `versiones` es la lista completa del presupuesto
//   // actual. `viendoVersionActual` en false significa que `presupuesto`
//   // tiene cargada una versión VIEJA (de solo lectura) en vez de la viva.
...
//   // Al subir un Excel para un proyecto que YA tiene presupuesto: como
//   // solo puede haber uno, SIEMPRE es una versión nueva de ESE presupuesto
//   // -- ya no hace falta preguntar "¿de cuál?" ni "¿o es aparte?", solo
//   // pedir el nombre de la versión. `itemsPendientesDeVersion` guarda lo
//   // que acaba de parsear el Excel mientras se pide el nombre;
//   // `versionPendienteDesdeImport` guarda el nombre ya confirmado, para
//   // que handleGuardar cree la versión vacía ANTES de insertar los ítems.
...
//     setError(null)
//     try {
//       const items = await cargarItemsDePresupuesto(presupuestoDbId)
//       setPresupuesto(items)
...
//       const [items, listaVersiones] = await Promise.all([
//         cargarItemsDePresupuesto(presupuestoDbId),
//         listarVersiones(presupuestoDbId),
//       ])
//       setPresupuesto(items)
...
//     setError(null)
//     try {
//       const items = await cargarItemsDePresupuesto(existente.id)
//       setPresupuesto(items)
...
//     setError(null)
//     let resultado: ItemPresupuesto[]
//     try {
//       resultado = await procesarPresupuesto(file)
//     } catch (e) {
//       // Incluye tanto ErrorFilasInvalidas (algún código no se pudo
//       // interpretar) como cualquier otro error de lectura del archivo --
//       // en ambos casos NO se agrega nada al presupuesto.
...
//       setItemsPendientesDeVersion(resultado)
//       return
//     }

//     setPresupuesto((prev) => [...prev, ...resultado])
//   }

//   function handleConfirmarNombreVersion() {
//     if (!itemsPendientesDeVersion || !nombreVersionDesdeImport.trim() || !presupuestoExistente)
//       return

//     setPresupuesto(itemsPendientesDeVersion)
...
//       const itemsNuevos = presupuesto.filter((i) => !i.guardado)
...
//             seguidos (1, 101, 10101) para numerar los items del presupuesto.
//           </p>
//         </div>
...
//                 size="sm"
//                 variant="outline"
//                 onClick={() => setItemsPendientesDeVersion(null)}
//               >
//                 Cancelar
//               </Button>
//             </div>
...
//                   </Button>
//                   <Button onClick={handleGuardar} disabled={guardando || !hayCambiosSinGuardar}>
//                     {guardando
//                       ? "Guardando..."
//                       : hayCambiosSinGuardar
...
// reconocen por alias, no por el nombre exacto del header.
//
// NUEVO: si el archivo trae una segunda hoja llamada "APU", también se
// parsea (parseApuSheet, lib/parse-apu-excel.ts) para el import masivo
// de APU -- ver handleFileSelected más abajo.
// ---------------------------------------------------------------------------
...
// Nombre de hoja para el import masivo de APU -- se busca sin distinguir
// mayúsculas/espacios, para no romper si alguien la nombra " Apu " o
// "apu" en vez de "APU" exacto.
...
export default function Presupuestos() {
  ...
  // -------------------------------------------------------------------
  // Import masivo de APU desde la hoja "APU" del Excel.
  //
  // Flujo (decisión de negocio -- ver nota larga en actions.ts): YA NO
  // se recomienda un APU completo de otro ítem/proyecto. Se guarda de
  // una vez (presupuesto + ítems + APU con lo que ya matcheó
  // automático), y lo ambiguo queda pendiente en apu_import_revision.
  //
  // La revisión de lo pendiente es un DIÁLOGO (no una pestaña -- ese
  // enfoque no funcionó, ver HANDOFF_import_apu.md) que se puede cerrar
  // sin terminar (los datos ya están guardados de todos modos) y
  // REABRIR después -- por eso `itemIdsParaRevisar` no depende de un
  // loteImportId, se arma con los ids de ítems que tengan algo
  // pendiente/rechazado en cualquier momento.
  //
  // La tabla del presupuesto NO se muestra mientras `guardandoImportApu`
  // está en true (matching + guardado ahora es una sola llamada) -- se
  // ve un spinner en su lugar, para no dar la impresión de que ya
  // terminó cuando todavía está corriendo.
  // -------------------------------------------------------------------
  const [guardandoImportApu, setGuardandoImportApu] = useState(false)
  // progreso del procesamiento por tandas -- null mientras no hay un
  // import en curso, o mientras handleGuardar (el insert de los ítems en
  // sí, antes de empezar el matching) todavía no terminó.
  const [progresoImport, setProgresoImport] = useState<{ procesados: number; total: number } | null>(
    null
  )
  // estado de APU por ítem ("listo"/"pendiente"/"rechazado") -- pinta la
  // tabla. Solo aparecen acá los ítems que vinieron de un import (los
  // demás no tienen entrada, PresupuestoTable los trata como neutros).
  ...
  // true cuando el diálogo se abrió con "Revisar pendientes" (todo lo
  // pendiente del presupuesto) -- en ese modo, la lista de ítems del
  // diálogo debe CRECER sola si terminan más tandas en el fondo mientras
  // está abierto. Si se abrió para un ítem puntual (click en una
  // etiqueta amarilla/roja de la tabla), queda acotado solo a ese ítem
  // -- no se infla con lo demás.
  ...
  // Trae valor_unitario/valor_total actualizados y los mezcla en
  // `presupuesto` -- resolver algo en el diálogo (elegir un insumo o una
  // categoría de mano de obra) SÍ recalcula el valor del lado del
  // servidor, pero refrescarEstadosApu solo traía los colores, nunca el
  // precio -- la tabla se quedaba mostrando el valor viejo hasta
