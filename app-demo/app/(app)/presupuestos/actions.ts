...
// /**
//  * Busca CATEGORÍAS de equipo/maquinaria parecidas al TEXTO DE LA LÍNEA
//  * del Excel (ej. "Retroexcavadora 320 sobre orugas") -- a diferencia de
//  * mano de obra, acá SÍ importa el texto de la línea (identifica la
//  * máquina), así que reusa el mismo prefiltro SQL + motor de similitud
//  * que buscarInsumosSimilares (arriba), contra equipo_categorias en vez
//  * de maestro_insumos.
//  */
// export async function buscarEquipoSimilares(termino: string, umbral = 0.3): Promise<CategoriaEquipo[]> {
//   if (!termino || termino.trim().length < 2) return []

//   const supabase = await createClient()

//   const { data, error } = await supabase.rpc("buscar_equipo_candidatos", {
//     p_termino: termino.trim(),
//     // Mismo razonamiento que buscar_insumos_candidatos: el RPC ya ordena
//     // por distancia de trigram, así que 50 alcanza sin sobrecargar el
//     // motor TF-IDF/Jaccard/Levenshtein en TS con todo el catálogo.
//     p_limite: 50,
//   })
...
// /**
//  * Análoga a agregarInsumoApu -- equipo SÍ multiplica por cantidad y
//  * rendimiento (a diferencia de mano de obra), así que la firma calca la
//  * de agregarInsumoApu, no la de agregarManoObraApu.
//  */
// export async function agregarEquipoApu(input: {
//   apuId: string
...
//   if (categoria.valor_unitario == null) {
//     throw new Error(
//       "Este equipo todavía no tiene precio definido -- ingresa el valor en el catálogo antes de asignarlo."
//     )
//   }

//   const { error: errorInsert } = await supabase.from("item_apu").insert({
//     apu_id: input.apuId,
//     equipo_categoria_id: input.equipoCategoriaId,
//     cantidad: input.cantidad,
//     rendimiento: input.rendimiento ?? 1,
//     tipo: "EQUIPO",
//   })
...
// // ---------------------------------------------------------------------------
// // Solicitudes de CATEGORÍA de equipo nueva -- mismo patrón que las
// // solicitudes de mano de obra (crear/listar/aprobar/rechazar), contra
// // equipo_categorias. Reusa el scope admin_mano_obra a propósito (decisión
// // del usuario: no crear admin_equipo aparte).
// // ---------------------------------------------------------------------------

// export type SolicitudEquipoInput = {
//   descripcion: string
//   grupoSugerido?: string | null
//   valorPropuesto?: number | null
//   presupuestoItemId?: string | null
...
// export type AprobarSolicitudEquipoInput = {
//   solicitudId: string
//   valorUnitario: number
//   grupo?: string | null
//   unidad: string
...
//   input: AprobarSolicitudEquipoInput
// ): Promise<{ id: string; categoria: string; grupo: string | null; valorUnitario: number; unidad: string }> {
//   await requerirScope("admin_mano_obra")
//   const supabase = await createClient()
...
//   }
//   if (!input.unidad || !input.unidad.trim()) {
//     throw new Error("Falta la unidad -- es importante para saber cómo aplicar el valor.")
//   }
...
// export async function rechazarSolicitudEquipo(solicitudId: string, motivo?: string) {
//   await requerirScope("admin_mano_obra")
//   const supabase = await createClient()
...
//       // esta línea) -- ver matchearManoDeObraApuImport más abajo.
//       if (linea.tipo === "MO") continue
//       // Equipo TAMPOCO -- se busca aparte, contra equipo_categorias (ver
//       // matchearEquipoApuImport más abajo). Antes matcheaba acá como
//       // cualquier material -- se sacó a propósito (decisión de negocio).
//       if (linea.tipo === "EQUIPO") continue
//       // "Herramienta menor X%" tampoco -- no es un insumo, es un cargo
//       // calculado como % de la mano de obra del mismo ítem (ver
//       // extraerPorcentajeHerramientaMenor arriba). Buscarla en el
//       // maestro no tiene sentido y podría auto-matchear mal contra un
...
// /**
//  * Matching de equipo -- por DESCRIPCIÓN ÚNICA DE LÍNEA (como insumos),
//  * no por ítem (como mano de obra) -- ver nota de diseño en
//  * ResolucionEquipo en lib/apu-import-types.ts. Auto-match SÍ permitido,
//  * con el mismo umbral que insumos (UMBRAL_AUTO_MATCH). Excluye
//  * "Herramienta menor X%" -- comparte tipo "EQUIPO" en el Excel de
//  * origen pero es un cargo calculado, no una máquina real (mismo
//  * criterio que ya usa matchearInsumosApuImport).
//  */
// export async function matchearEquipoApuImport(bloques: BloqueApuInput[]): Promise<ResolucionEquipo[]> {
...
//   return procesarEnLotes(Array.from(unicas.values()), 15, async (original) => {
//     const candidatos = await buscarEquipoSimilares(original, 0.3)

//     const mejor = candidatos[0]
//     const tienePlaceholder = !!mejor && mejor.valorUnitario == null
//     const score = mejor ? Math.round(mejor.similitud * 100) : 0
//     const esAutoMatch = !!mejor && score >= UMBRAL_AUTO_MATCH && !tienePlaceholder
...
//       descripcionOriginal: original,
//       estado: esAutoMatch ? "auto_match" : "requiere_revision",
//       equipoIdAsignado: esAutoMatch ? mejor!.id : null,
//       candidatosSugeridos: candidatos,
//       precioPlaceholder: tienePlaceholder,
//     }
//   })
...
//   bloques: BloqueApuInput[]
// ): Promise<ResultadoGuardarImportApu> {
//   const [resoluciones, resolucionesManoObra, resolucionesEquipo] = await Promise.all([
//     matchearInsumosApuImport(bloques),
//     matchearManoDeObraApuImport(bloques),
//     matchearEquipoApuImport(bloques),
//   ])
//   const bloquesConResolucion = combinarBloquesConResoluciones(
//     bloques,
//     resoluciones,
//     resolucionesManoObra,
//     resolucionesEquipo
//   )
//   return guardarImportApuConRevision(loteImportId, presupuestoItemIdPorCodigo, bloquesConResolucion)
// }
...
//   // ---- 3c. Insertar TODAS las líneas de EQUIPO auto_match de una vez ----
//   // Mismo patrón que el bloque 3 de arriba (insumos), pero contra
//   // equipo_categorias. Antes las líneas tipo "EQUIPO" pasaban por el
//   // bloque 3 como si fueran insumos -- ahora tienen su propio catálogo.
//   type LineaCandidataEquipo = { apuId: string; equipoId: string; cantidad: number }
//   const candidatasEquipoAutoMatch: LineaCandidataEquipo[] = []
//   for (const b of bloquesValidos) {
//     const apuId = apuIdPorCodigoItem.get(b.codigoItem)!
//     for (const linea of b.lineas) {
//       if (linea.tipo === "EQUIPO" && linea.estado === "auto_match" && linea.equipoIdAsignado) {
//         candidatasEquipoAutoMatch.push({ apuId, equipoId: linea.equipoIdAsignado, cantidad: linea.cantidad })
//       }
//     }
//   }

//   const equipoIdsUnicos = Array.from(new Set(candidatasEquipoAutoMatch.map((c) => c.equipoId)))
//   const infoEquipoPorId = new Map<string, { valor_unitario: number | null }>()

//   for (let i = 0; i < equipoIdsUnicos.length; i += 200) {
//     const lote = equipoIdsUnicos.slice(i, i + 200)
//     const { data, error } = await supabase.from("equipo_categorias").select("id, valor_unitario").in("id", lote)
//     if (error) throw new Error(error.message)
//     for (const row of data ?? []) infoEquipoPorId.set(row.id, { valor_unitario: row.valor_unitario })
//   }

//   const claveEquipo = (apuId: string, equipoId: string) => `${apuId}::${equipoId}`
//   const candidatasEquipoValidas: LineaCandidataEquipo[] = []
//   // Catálogo nuevo, sin datos todavía -- cualquier categoría sin
//   // valor_unitario cae a "pendiente" en vez de auto_match (no hay
//   // convención de "precio placeholder" acá como con insumos, solo null).
//   const clavesEquipoConPrecioPlaceholder = new Set<string>()

//   for (const c of candidatasEquipoAutoMatch) {
//     const info = infoEquipoPorId.get(c.equipoId)
//     if (!info || info.valor_unitario == null) {
//       clavesEquipoConPrecioPlaceholder.add(claveEquipo(c.apuId, c.equipoId))
//       continue
//     }
//     candidatasEquipoValidas.push(c)
//   }

//   const itemApuIdPorClaveEquipo = new Map<string, string>()
//   for (let i = 0; i < candidatasEquipoValidas.length; i += TAMANO_LOTE_ITEM_APU) {
//     const lote = candidatasEquipoValidas.slice(i, i + TAMANO_LOTE_ITEM_APU)
//     const filas = lote.map((c) => ({
//       apu_id: c.apuId,
//       equipo_categoria_id: c.equipoId,
//       cantidad: c.cantidad,
//       rendimiento: 1,
//       tipo: "EQUIPO",
//     }))
...
//       .from("item_apu")
//       .insert(filas)
//       .select("id, apu_id, equipo_categoria_id")
//     if (error) throw new Error(error.message)
//     for (const fila of insertadas ?? []) {
//       itemApuIdPorClaveEquipo.set(claveEquipo(fila.apu_id, fila.equipo_categoria_id), fila.id)
//     }
//   }
...
//       let estado: "auto_match" | "pendiente" = "pendiente"
//       let insumoIdGuardado: string | null = null
//       let equipoIdGuardado: string | null = null
...
//           insumo_id_asignado: null,
//           mano_obra_categoria_id_asignado: null,
//           equipo_categoria_id_asignado: null,
//           item_apu_id: null,
//         })
//         continue
//       }
...
//         // Líneas de equipo real (herramienta menor ya se descartó arriba
//         // -- comparte tipo "EQUIPO" en el Excel de origen pero se maneja
//         // aparte). Mismo criterio que insumos: si auto-matcheó Y no cayó
//         // en la lista de "sin precio", ya está insertado en item_apu
//         // (paso 3c) y se marca resuelto de una vez.
//         if (linea.estado === "auto_match" && linea.equipoIdAsignado) {
//           const claveLineaEquipo = claveEquipo(apuId, linea.equipoIdAsignado)
//           const idInsertado = itemApuIdPorClaveEquipo.get(claveLineaEquipo)
//           if (idInsertado && !clavesEquipoConPrecioPlaceholder.has(claveLineaEquipo)) {
//             itemApuId = idInsertado
//             estado = "auto_match"
//             equipoIdGuardado = linea.equipoIdAsignado
//           }
//         }
...
//           unidad: linea.unidad,
//           cantidad: linea.cantidad,
//           candidatos: linea.candidatosEquipoSugeridos,
//           estado,
//           insumo_id_asignado: null,
//           mano_obra_categoria_id_asignado: null,
//           equipo_categoria_id_asignado: equipoIdGuardado,
//           item_apu_id: itemApuId,
//         })
//         continue
//       }
...
//     unidad: fila.unidad,
//     cantidad: Number(fila.cantidad),
//     // "MO" trae CategoriaManoObra[], "EQUIPO" trae CategoriaEquipo[],
//     // cualquier otro tipo trae CandidatoInsumo[] -- ver el discriminador
//     // en FilaRevisionImport.
//     candidatos: (fila.candidatos ?? []) as CandidatoInsumo[] | CategoriaManoObra[] | CategoriaEquipo[],
//     estado: fila.estado,
//     insumoIdAsignado: fila.insumo_id_asignado,
//     manoObraCategoriaIdAsignado: fila.mano_obra_categoria_id_asignado,
//     equipoCategoriaIdAsignado: fila.equipo_categoria_id_asignado,
//     itemApuId: fila.item_apu_id,
//     motivoRechazo:
...
// const SELECT_REVISION_CON_MOTIVO =
//   "id, lote_import_id, presupuesto_item_id, apu_id, descripcion_original, tipo, unidad, cantidad, candidatos, estado, insumo_id_asignado, mano_obra_categoria_id_asignado, equipo_categoria_id_asignado, item_apu_id, presupuesto_items(codigo, descripcion), solicitudes_insumos(motivo_rechazo), solicitudes_mano_obra(motivo_rechazo), solicitudes_equipo(motivo_rechazo)"
...
// export async function resolverLineaRevision(input: {
//   revisionId: string
//   accion: "maestro" | "solicitud" | "mano_obra" | "solicitud_mano_obra" | "equipo" | "solicitud_equipo"
//   insumoId?: string
//   manoObraCategoriaId?: string
//   equipoCategoriaId?: string
// }): Promise<void> {
...
//       .select("id")
//       .eq("apu_id", fila.apu_id)
//       .eq("equipo_categoria_id", input.equipoCategoriaId)
//       .maybeSingle()
...
//     // 320") -- igual que insumos, porque el matching de equipo también
//     // es por texto de línea, no por ítem.
//     const solicitud = await crearSolicitudEquipo({
//       descripcion: fila.descripcion_original,
//       presupuestoItemId: fila.presupuesto_item_id,
//     })
...
//         .select("id")
//         .eq("apu_id", fila.apu_id)
//         .eq("equipo_categoria_id", r.equipoCategoriaId)
//         .maybeSingle()
...
//   // DEDUPLICADAS por descripción, igual que "solicitud" (insumo) más
//   // abajo -- a diferencia de mano de obra, el mismo equipo SÍ puede
//   // repetirse literalmente en muchos ítems (ej. "Retroexcavadora 320" en
//   // 30 ítems distintos), así que aplica el mismo criterio de dedup.
//   const resolucionesSolicitudEquipo = resoluciones.filter((r) => r.accion === "solicitud_equipo")
//   const gruposEquipoPorDescripcion = new Map<string, typeof resolucionesSolicitudEquipo>()

//   for (const r of resolucionesSolicitudEquipo) {
//     const fila = filaPorId.get(r.revisionId)
//     if (!fila || fila.estado === "resuelto") continue
//     const clave = fila.descripcion_original.trim().toLowerCase()
//     const grupo = gruposEquipoPorDescripcion.get(clave) ?? []
//     grupo.push(r)
//     gruposEquipoPorDescripcion.set(clave, grupo)
//   }

//   await procesarEnLotes(Array.from(gruposEquipoPorDescripcion.values()), 15, async (grupo) => {
//     const primeraFila = filaPorId.get(grupo[0].revisionId)!
//     try {
//       const solicitud = await crearSolicitudEquipo({
//         descripcion: primeraFila.descripcion_original,
...
// /**
//  * Análoga a editarLineaAutoMatch, para corregir un auto_match de equipo
//  * (pestaña "Automáticos" de Equipo).
//  */
// export async function editarLineaAutoMatchEquipo(input: {
//   revisionId: string
...
//   await agregarEquipoApu({ apuId: fila.apu_id, equipoCategoriaId: input.nuevoEquipoId, cantidad: Number(fila.cantidad) })

//   const { data: filaInsertada } = await supabase
//     .from("item_apu")
//     .select("id")
//     .eq("apu_id", fila.apu_id)
//     .eq("equipo_categoria_id", input.nuevoEquipoId)
//     .maybeSingle()

//   const { error: errorUpdate } = await supabase
//     .from("apu_import_revision")
//     .update({ equipo_categoria_id_asignado: input.nuevoEquipoId, item_apu_id: filaInsertada?.id ?? null })
//     .eq("id", input.revisionId)
//   if (errorUpdate) throw new Error(errorUpdate.message)
...
// Catálogo COMPLETO de mano de obra, sin filtro -- pensado para
// cachearse una sola vez del lado del cliente y filtrar ahí en cada
// tecla, en vez de un roundtrip al servidor por cada pausa al escribir
// (que es lo que hacía buscarManoObraSimilares usada como buscador
// libre -- de ahí la lentitud reportada). El catálogo es chico (~150
// filas hoy) así que traerlo completo una vez es más barato que 150
// búsquedas de red durante la vida de un solo diálogo abierto.
export async function listarManoObraCategoriasCompleto(): Promise<CategoriaManoObra[]> {
  const supabase = await createClient()
...
export type ItemApu = {
  ...
  insumoId: string | null
  manoObraCategoriaId: string | null
  equipoCategoriaId: string | null
  transportePrecioId: string | null
  // true SOLO para la línea calculada de "Herramienta menor" (%
  // sobre mano de obra) -- no tiene fila de catálogo propia (todas las
  // demás IDs de arriba quedan null), así que sin esta bandera no hay
  // forma estructural de distinguirla de un insumo/línea huérfana.
...
function mapearApu(fila: any, usos: number): ApuDeItem {
  ...
  return {
    ...
    usos,
    items: itemsCrudos.map((it: any) => {
      // Una línea es de insumo, de mano de obra, de equipo, de
      // transporte, O de "herramienta menor" (% de mano de obra) --
      // nunca dos de las cinco (mismo check constraint de la base).
      // "categoria" hace de descripción para mano de obra y equipo;
      // transporte usa su propia descripcion_original (no viene de
      // ningún catálogo); para herramienta menor no hay fila de
      // catálogo, se arma la descripción con el porcentaje guardado.
      const insumo = it.maestro_insumos
      const manoObra = it.mano_obra_categorias
      const equipo = it.equipo_categorias
      const transportePrecio = it.transporte_precios
      ...
      if (it.porcentaje_mano_obra != null) {
        ...
        return {
          ...
          insumoId: null,
          manoObraCategoriaId: null,
          equipoCategoriaId: null,
          transportePrecioId: null,
      ...
      return {
        ...
        insumoId: it.insumo_id,
        manoObraCategoriaId: it.mano_obra_categoria_id,
        equipoCategoriaId: it.equipo_categoria_id,
        transportePrecioId: it.transporte_precio_id,
        ...
        vrUnitario:
          insumo?.vr_unitario ?? manoObra?.valor_unitario ?? equipo?.valor_unitario ??
          transportePrecio?.valor_unitario ?? null,
        // Para mano de obra y transporte, cantidad/rendimiento se
        // muestran en 1 -- igual que arriba con herramienta menor, el
        // valor real NO depende de la cantidad/rendimiento que traía la
        // línea vieja del Excel detallado (se ignoran, ver
        // recalcular_valor_apu -- transporte suma valor_unitario sin
        // multiplicar por nada, mismo criterio que mano de obra).
        // Mostrar el número original ahí sería engañoso -- parecería
        // que participa en el cálculo cuando no es así. Equipo SÍ
        // multiplica por cantidad × rendimiento (fórmula marcada como
        // "revisar después" en el handoff), así que se muestra real.
        cantidad: manoObra || transportePrecio ? 1 : Number(it.cantidad),
        rendimiento: manoObra || transportePrecio ? 1 : Number(it.rendimiento ?? 1),
        tipo: insumo?.tipo ?? it.tipo ?? null,
      }
    }),
  }
...
const SELECT_APU =
  "id, codigo, descripcion, item_apu(id, insumo_id, mano_obra_categoria_id, porcentaje_mano_obra, equipo_categoria_id, transporte_precio_id, cantidad, rendimiento, tipo, maestro_insumos(codigo, descripcion, u_m, tipo, vr_unitario), mano_obra_categorias(categoria, grupo, unidad, valor_unitario), equipo_categorias(categoria, grupo, unidad, valor_unitario), transporte_precios(descripcion_original, unidad, valor_unitario))"
...
export async function obtenerApusParaExportar(apuIds: string[]): Promise<ApuExport[]> {
  ...
  const data = await seleccionarEnLotesPorIds(apuIds, 200, async (lote) => {
    const { data, error } = await supabase
      ...
      item_apu(
        ...
        transporte_precios(descripcion_original, unidad, valor_unitario)
      )
    `
      )
      .in("id", lote)
  ...
  return (data ?? []).map((apu: any) => {
    ...
    const lineas: LineaApuExport[] = itemsCrudos.map((item: any) => {
      const insumo = item.maestro_insumos
      const manoObra = item.mano_obra_categorias
      const equipo = item.equipo_categorias
      const transportePrecio = item.transporte_precios
      ...
      // Mano de obra y transporte: cantidad/rendimiento se exportan en
      // 1 -- el valor real no depende de lo que traía la línea vieja
      // del Excel detallado, mostrar ese número ahí sería engañoso
      // (mismo criterio que mapearApu / recalcular_valor_apu). Equipo
      // SÍ multiplica por cantidad × rendimiento.
      const cantidad = manoObra || transportePrecio ? 1 : Number(item.cantidad ?? 0)
      const rendimiento = manoObra || transportePrecio ? 1 : Number(item.rendimiento ?? 1)
      const valorUnitario =
        insumo?.vr_unitario != null
          ? Number(insumo.vr_unitario)
          : manoObra?.valor_unitario != null
            ...
            : equipo?.valor_unitario != null
              ? Number(equipo.valor_unitario)
              : transportePrecio?.valor_unitario != null
                ? Number(transportePrecio.valor_unitario)
                : null
...
// Normaliza para comparar nombres de categoría sin importar
// mayúsculas o espacios extra -- "Retroexcavadora" y "retroexcavadora "
// deben contar como la MISMA categoría.
function normalizarNombreCategoria(s: string): string {
  return s.trim().toLowerCase().replace(/\s+/g, " ")
}

// Antes de crear una categoría nueva de mano de obra/equipo, revisa si
// ya existe una con el mismo nombre normalizado -- evita el caso real
// que motivó esto: "Retroexcavadora" terminando 3 veces en el catálogo
// porque 3 solicitudes distintas pidieron básicamente lo mismo con
// mayúsculas/espacios distintos. Solo bloquea coincidencia EXACTA
// normalizada -- "Retroexcavadora 320" y "Retroexcavadora 330" son
// máquinas distintas aunque se parezcan, y el admin ya vio la tabla de
// candidatos (buscarEquipoSimilares/buscarManoObraSimilares) antes de
// decidir que esto era "nuevo" -- este check es la última red de
// seguridad, no un reemplazo de esa revisión, así que no vale la pena
// ponerlo más agresivo (bloquear por similitud en vez de exactitud) y
// arriesgar falsos positivos entre máquinas/actividades parecidas pero
// distintas.
//
// NOTA: maestro_insumos (insumos) NO tiene este mismo check -- solo
// tiene UNIQUE(codigo), que es un código secuencial, no la descripción.
// O sea que insumos tampoco está 100% protegido contra este mismo tipo
// de duplicado -- si quieres el mismo check ahí, es la misma idea,
// avísame y lo agrego en aprobarSolicitudInsumo también.
async function buscarCategoriaExistentePorNombre(
  supabase: Awaited<ReturnType<typeof createClient>>,
...
export async function aprobarSolicitudManoObra(
  ...
  const existente = await buscarCategoriaExistentePorNombre(supabase, "mano_obra_categorias", solicitud.descripcion)
  if (existente) {
    throw new Error(
      `Ya existe una categoría de mano de obra llamada "${existente.categoria}" -- rechaza esta ` +
        `solicitud y dile a quien la pidió que use esa en vez de crear una nueva.`
    )
  }
...
export async function aprobarSolicitudEquipo(
  ...
  const existente = await buscarCategoriaExistentePorNombre(supabase, "equipo_categorias", solicitud.descripcion)
  if (existente) {
    throw new Error(
      `Ya existe un equipo llamado "${existente.categoria}" -- rechaza esta solicitud y dile a ` +
        `quien la pidió que use ese en vez de crear uno nuevo.`
    )
  }
...
export async function matchearInsumosApuImport(
  ...
  for (const bloque of bloques) {
    for (const linea of bloque.lineas) {
      ...
      // cualquier material -- se sacó a propósito (decisión de negocio).
      if (linea.tipo === "EQUIPO") continue
      // Transporte TAMPOCO -- no tiene catálogo ni matching de ningún
      // tipo, se resuelve por Excel (ver obtenerPendientesTransporte /
      // importarPreciosTransporte más abajo). Antes matcheaba acá como
      // cualquier material -- mismo error que tenía Equipo.
      if (linea.tipo === "TRANSPORTE") continue
      // "Herramienta menor X%" tampoco -- no es un insumo, es un cargo
      // calculado como % de la mano de obra del mismo ítem (ver
      // extraerPorcentajeHerramientaMenor arriba). Buscarla en el
      // maestro no tiene sentido y podría auto-matchear mal contra un
 ...
 * criterio que ya usa matchearInsumosApuImport).
 */
// "Volqueta" viene tipeada como EQUIPO en el Excel de origen (así la
// cargan los ingenieros al armar el APU), pero conceptualmente es
// transporte -- se cobra por distancia/viaje, no por hora de máquina
// como el resto del catálogo de equipo. Decisión de negocio: se
// reclasifica a tipo "TRANSPORTE" ANTES de que corra cualquier
// matching (ver reclasificarVolquetaComoTransporte más abajo) -- así
// cae sola en el mismo pipeline de transporte que ya existe
// (apu_import_revision con tipo "TRANSPORTE", pendiente hasta que se
// suba el Excel con el precio -- ver obtenerPendientesTransporte) en
// vez de necesitar un mecanismo aparte. matchearEquipoApuImport ni se
// entera: para cuando corre, estas líneas ya no tienen tipo "EQUIPO".
const RE_VOLQUETA = /volquet/i

// Devuelve una copia de `bloques` con las líneas de "Volqueta"
// reclasificadas de tipo "EQUIPO" a "TRANSPORTE". NO muta el array
// original -- page.tsx reusa `bloques` para el botón de "reintentar"
// (bloquesParaReintentar) si algo falla a mitad de import, así que
// mutar in-place podría hacer que un reintento ya no reconozca esas
// líneas como las mismas.
function reclasificarVolquetaComoTransporte(bloques: BloqueApuInput[]): BloqueApuInput[] {
  return bloques.map((bloque) => ({
    ...bloque,
    lineas: bloque.lineas.map((linea) =>
      linea.tipo === "EQUIPO" && RE_VOLQUETA.test(linea.descripcion)
        ? { ...linea, tipo: "TRANSPORTE" as const }
        : linea
    ),
  }))
}
...
export async function matchearYGuardarImportApu(
  loteImportId: string,
  presupuestoItemIdPorCodigo: Record<string, string>,
  bloquesOriginales: BloqueApuInput[]
): Promise<ResultadoGuardarImportApu> {
  // Reclasifica "Volqueta" (EQUIPO -> TRANSPORTE) antes de que arranque
  // cualquier matching -- ver reclasificarVolquetaComoTransporte arriba.
  const bloques = reclasificarVolquetaComoTransporte(bloquesOriginales)
  const [resoluciones, resolucionesManoObra, resolucionesEquipo] = await Promise.all([
    matchearInsumosApuImport(bloques),
    matchearManoDeObraApuImport(bloques),
    matchearEquipoApuImport(bloques),
...
async function guardarImportApuConRevision(
  ...
  // equipo_categorias. Antes las líneas tipo "EQUIPO" pasaban por el
  // bloque 3 como si fueran insumos -- ahora tienen su propio catálogo.
  type LineaCandidataEquipo = { apuId: string; equipoId: string; cantidad: number; rendimiento: number }
  const candidatasEquipoAutoMatch: LineaCandidataEquipo[] = []
  for (const b of bloquesValidos) {
    const apuId = apuIdPorCodigoItem.get(b.codigoItem)!
    for (const linea of b.lineas) {
      if (linea.tipo === "EQUIPO" && linea.estado === "auto_match" && linea.equipoIdAsignado) {
        candidatasEquipoAutoMatch.push({
          apuId,
          equipoId: linea.equipoIdAsignado,
          cantidad: linea.cantidad,
          rendimiento: linea.rendimiento ?? 1,
        })
      }
  ...
  for (const b of bloquesValidos) {
    ...
    for (const linea of b.lineas) {
      ...
      // OJO: este check va PRIMERO, antes de cualquier branch por tipo --
      // "Herramienta menor X%" es un cargo calculado, nunca una línea
      // real de insumo/MO/equipo, sin importar qué tipo traiga la
      // columna del Excel. El bug real (reportado: mano de obra
      // duplicada en TODOS los ítems que la tienen) era que el branch
      // `if (linea.tipo === "MO")` de abajo corría ANTES que este check
      // -- entonces una línea como "Herramienta menor 10% M.O. AA-3"
      // (tipo MO en el Excel, pasa perfectamente el regex de abajo)
      // caía en el branch de MO como si fuera una cuadrilla real,
      // pendiente de que alguien le eligiera categoría. Como mano de
      // obra matchea por NOMBRE DEL ÍTEM (no por texto de línea), esa
      // línea sugería la MISMA categoría que la línea real de mano de
      // obra del mismo ítem -- y si se resolvía, insertaba una SEGUNDA
      // fila real en item_apu con la misma categoría, aunque el 10% ya
      // se había insertado bien en el paso 3b. Resultado visible: la
      // categoría de mano de obra aparecía 2 veces en el APU, en
      // básicamente todos los ítems que traían herramienta menor
      // tipeada como MO en vez de EQUIPO (algo que si pasa en algunos
      // Excels de origen, aunque el comentario original decía que
      // "siempre" era EQUIPO). El check de abajo para EQUIPO y
      // TRANSPORTE ya estaba bien ordenado (percentage-check antes);
      // solo faltaba moverlo antes del branch de MO también.
      const porcentajeHerramientaMenor = extraerPorcentajeHerramientaMenor(linea.descripcion)
      if (porcentajeHerramientaMenor !== null) {
        // Nunca pendiente -- es un cargo calculado (% de la mano de obra
        // del mismo APU), no algo que el ingeniero tenga que elegir. Ya
      ...
      if (linea.tipo === "TRANSPORTE") {
        // Sin candidatos, sin auto-match, sin item_apu -- queda SIEMPRE
        // pendiente hasta que se suba el Excel con el precio (ver
        // obtenerPendientesTransporte/importarPreciosTransporte). No hay
        // "sin precio" que chequear acá porque no hay catálogo del que
        // sacar un precio placeholder -- la única fuente de valor es lo
        // que el ingeniero escriba en el Excel.
        lineasPendientes++
        filasRevision.push({
          lote_import_id: loteImportId,
          presupuesto_item_id: presupuestoItemId,
      ...
      if (linea.tipo === "EQUIPO") {
        // Líneas de equipo real (herramienta menor ya se descartó arriba
        // -- comparte tipo "EQUIPO" en el Excel de origen pero se maneja
        // aparte, y "Volqueta" ya se reclasificó a TRANSPORTE más arriba
        // en matchearYGuardarImportApu, así que tampoco llega hasta acá
        // como EQUIPO). Mismo criterio que insumos: si auto-matcheó Y no
        // cayó en la lista de "sin precio", ya está insertado en item_apu
        // (paso 3c) y se marca resuelto de una vez.
        if (linea.estado === "auto_match" && linea.equipoIdAsignado) {
          const claveLineaEquipo = claveEquipo(apuId, linea.equipoIdAsignado)
          const idInsertado = itemApuIdPorClaveEquipo.get(claveLineaEquipo)
...
function mapearFilaRevision(fila: any): FilaRevisionImport {
  // El embed de transporte_precios puede volver como array u objeto
  // según cómo PostgREST infiera la relación 1:1 -- se maneja
  // defensivamente en vez de asumir una forma fija.
  const transportePrecio = Array.isArray(fila.transporte_precios)
    ? fila.transporte_precios[0]
    : fila.transporte_precios
  ...
  return {
    ...
    cantidad: Number(fila.cantidad),
    // "MO" trae CategoriaManoObra[], "EQUIPO" trae CategoriaEquipo[],
    // "TRANSPORTE" siempre [], cualquier otro tipo trae CandidatoInsumo[]
    // -- ver el discriminador en FilaRevisionImport.
    candidatos: (fila.candidatos ?? []) as CandidatoInsumo[] | CategoriaManoObra[] | CategoriaEquipo[],
    estado: fila.estado,
    insumoIdAsignado: fila.insumo_id_asignado,
    manoObraCategoriaIdAsignado: fila.mano_obra_categoria_id_asignado,
...
const SELECT_REVISION_CON_MOTIVO =
  "id, lote_import_id, presupuesto_item_id, apu_id, descripcion_original, tipo, unidad, cantidad, candidatos, estado, insumo_id_asignado, mano_obra_categoria_id_asignado, equipo_categoria_id_asignado, item_apu_id, presupuesto_items(codigo, descripcion), solicitudes_insumos(motivo_rechazo), solicitudes_mano_obra(motivo_rechazo), solicitudes_equipo(motivo_rechazo), transporte_precios(id, valor_unitario)"
...
export async function resolverLineaRevision(input: {
  ...
  const { data: fila, error: errorLectura } = await supabase
    .from("apu_import_revision")
    .select(
      "apu_id, presupuesto_item_id, descripcion_original, tipo, unidad, cantidad, rendimiento, estado, presupuesto_items(descripcion)"
    )
    .eq("id", input.revisionId)
    .single()
...
export async function editarLineaAutoMatch(input: {
  ...
  // "Corregir" al mismo insumo que ya tenía asignado no es una
  // corrección real -- pasa cuando alguien hace click de nuevo sobre la
  // fila ya seleccionada en la tabla de candidatos (bug reportado: eso
  // terminaba agregando el insumo 2 veces). Cortar acá evita el
  // insert+delete innecesario por completo, no solo el síntoma.
  if (fila.insumo_id_asignado === input.nuevoInsumoId) {
    return
  }

  const itemApuIdViejo = fila.item_apu_id

  // 1. Insertar la línea NUEVA primero -- se captura el id DIRECTO del
  // insert (antes se volvía a buscar por apu_id+insumo_id, pero esa
  // consulta es ambigua apenas hay 2 filas con el mismo insumo en el
  // mismo APU -- la vieja que se está por borrar Y la nueva recién
  // insertada) -- y .maybeSingle() fallaba en silencio (el error no se
  // revisaba), dejando item_apu_id en null. La próxima vez que se
  // "corrigiera" esa misma línea, ya no había itemApuIdViejo que borrar
  // -- los duplicados se quedaban para siempre. Usar el id del insert
  // directo elimina la ambigüedad de raíz.
  const { id: idNuevo } = await agregarInsumoApu({
    apuId: fila.apu_id,
  ...
  const { error: errorUpdate } = await supabase
    .from("apu_import_revision")
    .update({ insumo_id_asignado: input.nuevoInsumoId, item_apu_id: idNuevo })
    .eq("id", input.revisionId)
  if (errorUpdate) throw new Error(errorUpdate.message)
...
export async function editarLineaAutoMatchEquipo(input: {
  ...
  const { data: fila, error: errorLectura } = await supabase
    .from("apu_import_revision")
    .select("apu_id, presupuesto_item_id, cantidad, item_apu_id, equipo_categoria_id_asignado")
    .eq("id", input.revisionId)
    .single()

  if (errorLectura) throw new Error(errorLectura.message)

  // Mismo guard que editarLineaAutoMatch -- ver comentario ahí.
  if (fila.equipo_categoria_id_asignado === input.nuevoEquipoId) {
    return
  }
  ...
  const { id: idNuevo } = await agregarEquipoApu({
    apuId: fila.apu_id,
    equipoCategoriaId: input.nuevoEquipoId,
    cantidad: Number(fila.cantidad),
  })

  const { error: errorUpdate } = await supabase
    .from("apu_import_revision")
    .update({ equipo_categoria_id_asignado: input.nuevoEquipoId, item_apu_id: idNuevo })
    .eq("id", input.revisionId)
  if (errorUpdate) throw new Error(errorUpdate.message)
 ...
 */
// ---------------------------------------------------------------------------
// Transporte -- roundtrip por Excel, sin catálogo ni similitud (ver
// diseño acordado con el usuario). obtenerPendientesTransporte arma los
// datos para el Excel que se descarga (incluye el valor ya cargado, si
// lo hay, para que el mismo archivo sirva para corregir);
// importarPreciosTransporte procesa el Excel ya diligenciado.
// ---------------------------------------------------------------------------

export async function obtenerItemsConTransportePendiente(presupuestoItemIds: string[]): Promise<string[]> {
  if (presupuestoItemIds.length === 0) return []
...
export async function obtenerPendientesTransporte(
  presupuestoItemIds: string[]
): Promise<FilaExportTransporte[]> {
  if (presupuestoItemIds.length === 0) return []

  const supabase = await createClient()
  const resultado: FilaExportTransporte[] = []
  ...
  for (let i = 0; i < presupuestoItemIds.length; i += 200) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `
        id, descripcion_original, unidad, cantidad,
        presupuesto_items!inner (
          codigo, descripcion, unidad, cantidad,
          presupuestos ( proyectos ( nombre ) )
        ),
        transporte_precios ( valor_unitario )
      `
      )
      .eq("tipo", "TRANSPORTE")
      .in("presupuesto_item_id", lote)
    ...
    for (const fila of data ?? []) {
      const item = (fila as any).presupuesto_items
      const transportePrecioRaw = (fila as any).transporte_precios
      const transportePrecio = Array.isArray(transportePrecioRaw) ? transportePrecioRaw[0] : transportePrecioRaw

      resultado.push({
        revisionId: fila.id,
        proyectoNombre: item?.presupuestos?.proyectos?.nombre ?? null,
        itemCodigo: item?.codigo ?? "",
        itemDescripcion: item?.descripcion ?? "",
        itemUnidad: item?.unidad ?? null,
        itemCantidad: item?.cantidad ?? null,
        descripcionTransporte: fila.descripcion_original,
        unidadTransporte: fila.unidad,
        cantidadTransporte: Number(fila.cantidad),
        valorActual: transportePrecio?.valor_unitario ?? null,
      })
    }
  }
...
export async function importarPreciosTransporte(
  filas: FilaImportTransporte[]
): Promise<{ errores: { revisionId: string; mensaje: string }[]; actualizados: number; sinCambios: number }> {
  if (filas.length === 0) return { errores: [], actualizados: 0, sinCambios: 0 }

  const supabase = await createClient()
  const errores: { revisionId: string; mensaje: string }[] = []

  // 1. Validar formato ANTES de tocar la base -- no se adivina ni se
  // deja pasar un valor dudoso (mismo criterio que ErrorFilasInvalidas
  // en el import principal de page.tsx).
  const filasValidas: FilaImportTransporte[] = []
  for (const f of filas) {
    if (!f.revisionId) {
      errores.push({ revisionId: f.revisionId ?? "?", mensaje: "Falta el identificador de la línea (columna oculta del Excel)." })
      continue
    }
    if (typeof f.valorUnitario !== "number" || !Number.isFinite(f.valorUnitario) || f.valorUnitario < 0) {
      errores.push({ revisionId: f.revisionId, mensaje: "El valor no es un número válido (debe ser 0 o mayor)." })
      continue
    }
    filasValidas.push(f)
  }

  if (filasValidas.length === 0) return { errores, actualizados: 0, sinCambios: 0 }

  // 2. Traer las filas de apu_import_revision que el Excel dice resolver
  // -- el ID oculto es la ÚNICA fuente de verdad del match, nunca se
  // vuelve a matchear por texto. Un ID que no exista o no sea de tipo
  // TRANSPORTE se reporta como error explícito, no se ignora en
  // silencio (evita el "happy path bias" de asumir que el Excel subido
  // siempre está bien formado).
  const idsPedidos = filasValidas.map((f) => f.revisionId)
  const filaPorId = new Map<string, any>()
  for (let i = 0; i < idsPedidos.length; i += 200) {
    const lote = idsPedidos.slice(i, i + 200)
    const { data, error } = await supabase
      .from("apu_import_revision")
      .select(
        `
        id, apu_id, presupuesto_item_id, descripcion_original, unidad, tipo,
        presupuesto_items ( presupuestos ( proyecto_id ) )
      `
      )
      .in("id", lote)
    if (error) throw new Error(error.message)
    for (const f of data ?? []) filaPorId.set(f.id, f)
  }

  // 3. Traer los precios YA cargados -- para diffear en memoria y solo
  // escribir lo que de verdad cambió (decisión de diseño: nada de
  // "marcado de cambios" con estado extra, un solo fetch + comparación
  // en memoria alcanza a esta escala).
  const preciosExistentes = new Map<string, { id: string; valor_unitario: number }>()
  for (let i = 0; i < idsPedidos.length; i += 200) {
    const lote = idsPedidos.slice(i, i + 200)
    const { data, error } = await supabase
      .from("transporte_precios")
      .select("id, apu_import_revision_id, valor_unitario")
      .in("apu_import_revision_id", lote)
    if (error) throw new Error(error.message)
  ...
  const filasParaUpsert: {
    apu_import_revision_id: string
  ...
  for (const f of filasValidas) {
    const fila = filaPorId.get(f.revisionId)
    if (!fila) {
      errores.push({
        revisionId: f.revisionId,
        mensaje: "No se encontró esa línea -- ¿el Excel es de otro presupuesto o de una versión vieja?",
      })
      continue
    }
    if (fila.tipo !== "TRANSPORTE") {
      errores.push({ revisionId: f.revisionId, mensaje: "Esta línea no es de transporte." })
      continue
    }

    const existente = preciosExistentes.get(f.revisionId)
    if (existente && existente.valor_unitario === f.valorUnitario) {
      sinCambios++
      continue
    }
    if (!existente) idsNuevos.add(f.revisionId)

    filasParaUpsert.push({
      apu_import_revision_id: f.revisionId,
      proyecto_id: fila.presupuesto_items?.presupuestos?.proyecto_id ?? null,
  ...
  if (filasParaUpsert.length === 0) return { errores, actualizados: 0, sinCambios }

  // 4. Upsert por lotes -- el UNIQUE constraint en apu_import_revision_id
  // hace que esto sea "insertar si es nuevo, actualizar si ya existía"
  // sin tener que distinguir los dos casos en el UPDATE/INSERT en sí.
  const idPreciosPorRevisionId = new Map<string, string>()
  for (let i = 0; i < filasParaUpsert.length; i += 200) {
    const lote = filasParaUpsert.slice(i, i + 200)
    const { data, error } = await supabase
      .from("transporte_precios")
      .upsert(lote, { onConflict: "apu_import_revision_id" })
      .select("id, apu_import_revision_id")
    if (error) throw new Error(error.message)
    for (const p of data ?? []) idPreciosPorRevisionId.set(p.apu_import_revision_id, p.id)
  }

  // 5. Para las líneas NUEVAS (nunca habían tenido precio) hay que
  // crear el item_apu que las conecta al APU real -- las que ya tenían
  // precio (corrección) reusan el MISMO item_apu de antes, porque
  // apuntan al mismo transporte_precios.id, que solo cambió de valor.
  const filasNuevasParaItemApu = filasParaUpsert
    .filter((f) => idsNuevos.has(f.apu_import_revision_id))
    .map((f) => ({
      apu_id: filaPorId.get(f.apu_import_revision_id).apu_id as string,
      transporte_precio_id: idPreciosPorRevisionId.get(f.apu_import_revision_id)!,
      cantidad: 1,
      rendimiento: 1,
      tipo: "TRANSPORTE",
    }))

  const itemApuIdPorPrecioId = new Map<string, string>()
  const TAMANO_LOTE_ITEM_APU_TRANSPORTE = 500
  for (let i = 0; i < filasNuevasParaItemApu.length; i += TAMANO_LOTE_ITEM_APU_TRANSPORTE) {
    const lote = filasNuevasParaItemApu.slice(i, i + TAMANO_LOTE_ITEM_APU_TRANSPORTE)
    const { data, error } = await supabase.from("item_apu").insert(lote).select("id, transporte_precio_id")
    if (error) throw new Error(error.message)
    for (const row of data ?? []) itemApuIdPorPrecioId.set(row.transporte_precio_id, row.id)
  }
  ...
  for (const revisionId of idsNuevos) {
    ...
    const { error } = await supabase
      .from("apu_import_revision")
      .update({ estado: "resuelto", item_apu_id: itemApuId ?? null })
      .eq("id", revisionId)
    if (error) {
...
export type ItemsConEstadoApu = {
  ...
  // que estados/motivosRechazo ya viajan juntos con items.
  itemIdsTransportePendiente: string[]
}
