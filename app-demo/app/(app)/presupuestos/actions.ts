...
//   if (error) {
//     // 23505 = unique_violation -- ver constraint presupuestos_proyecto_id_unique
//     // (un proyecto solo puede tener un presupuesto; ver
//     // migracion_presupuesto_unico.sql). Si esto salta es porque algo no
//     // pasó primero por verPresupuestoDeProyecto, o hubo una carrera
//     // entre dos pestañas -- el mensaje le dice al usuario qué hacer en
//     // vez de mostrarle el texto crudo de Postgres.
//     if (error.code === "23505") {
//       throw new Error(
...
// // Pedir lista de proyectos
// export async function verProyectos() {
//   const supabase = await createClient()

//   console.time("verProyectos:getUser")

//   const {
//     data: { user },
//   } = await supabase.auth.getUser()

//   console.timeEnd("verProyectos:getUser")

//   if (!user) {
//     throw new Error("No autenticado.")
//   }

//   console.time("verProyectos:permisos")

//   const permisos = await obtenerPermisosUsuario(user.id)

//   console.timeEnd("verProyectos:permisos")

//   if (permisos.veTodosProyectos) {
//     console.time("verProyectos:todosLosProyectos")
...
//     console.timeEnd("verProyectos:todosLosProyectos")
...
//   console.time("verProyectos:proyectosPermitidos")
...
//   console.timeEnd("verProyectos:proyectosPermitidos")
...
// //   const permisos = await obtenerPermisosUsuario(user.id)  // Mira si es admin o no

// //   //Si tiene todos los permisos se buscan todos los proyectos para mostrarlos
// //   if (permisos.veTodosProyectos) {
// //     const { data, error } = await supabase
// //       .from("proyectos")
// //       .select("id, codigo, nombre")
...
// // El único presupuesto de un proyecto (o null si aún no tiene uno). Un
// // proyecto no puede tener más de uno -- constraint
// // presupuestos_proyecto_id_unique -- así que ya no hace falta ni
// // devolver una lista ni preguntarle al usuario "¿cuál de estos?".
// // Reemplaza a la vieja verPresupuestosDeProyecto (plural).
...
// // Carga los ítems de un presupuesto ya existente, en el mismo formato que
// // usa el estado del cliente -- para poder "continuar" un presupuesto en
// // vez de crear uno nuevo desde cero. Como el `id` del cliente y el `id`
// // real en presupuesto_items son el mismo UUID (ver AñadirItemPresuouesto),
// // esto simplemente los trae tal cual, todos marcados guardado: true.
// //
// // OJO con el orden: la tabla espera un orden JERÁRQUICO (cada padre
// // seguido inmediatamente de sus hijos, como sale del stack al importar
// // el Excel) -- un simple `.order("nivel")` de Postgres NO alcanza,
// // porque eso agrupa por nivel (todos los nivel 1, luego todos los
// // nivel 2 sin importar de qué padre son, etc.), no por jerarquía. Por
// // eso acá se reconstruye el árbol real por padre_id y se aplana en
// // orden jerárquico (DFS), usando el código para ordenar los hermanos
...
//   // BUG que arreglamos acá antes: esta función traía TODOS los ítems del
//   // presupuesto sin filtrar por versión (y sin traer apu_id -- ver commit
//   // anterior). Ahora delega en cargarVersion, que sí filtra por
//   // version_id, así que solo se ve la versión actual/editable.
//   return cargarVersion(presupuesto.version_actual_id)
// }

// // Reconstruye el orden jerárquico (DFS) a partir de padreId, en vez de
// // confiar en el orden en que Postgres devuelve las filas.
...
//   // por si algún ítem quedó con un padreId que no existe en el set
//   // (dato corrupto/huérfano) -- no lo perdemos, lo agregamos al final
//   // en vez de que desaparezca silenciosamente.
...
// // ---------------------------------------------------------------------------
// // Versiones del presupuesto. Cada presupuesto necesita al menos una
// // versión ("Versión inicial", numero=1) para que presupuesto_items.
// // version_id nunca quede null -- esta función la crea la primera vez que
// // alguien guarda algo (AñadirItemPresuouesto) si todavía no existe
// // ninguna, así un presupuesto recién creado no necesita un paso aparte.
// // ---------------------------------------------------------------------------
...
// // Crea una versión NUEVA Y VACÍA (a diferencia de crearNuevaVersion, que
// // duplica los ítems de la versión actual) -- para el caso de "subir un
// // Excel nuevo para un presupuesto que ya existe": el contenido de la
// // versión nueva es lo que traiga ese Excel, no una copia de la versión
// // anterior. La deja como version_actual_id de una vez, así que
// // AñadirItemPresuouesto (que usa obtenerOCrearVersionActual) inserta los
// // ítems del Excel directo en esta versión, sin mezclarlos con la anterior.
...
// // Crea una nueva versión duplicando TODOS los ítems de la versión actual
// // -- y sus APU (copiados a un apu_id nuevo por ítem, igual que
// // copiarApuParaItem/copiarApuStandalone), para que la versión anterior
// // quede como una foto congelada de verdad: si más adelante alguien edita
// // el precio de un insumo en el maestro y recalcula el APU de la versión
// // nueva, la versión vieja NO se mueve, porque ya tiene su propio apu_id
// // independiente.
...
//     .select("id, padre_id, nivel, codigo, descripcion, unidad, cantidad, valor_unitario, valor_total, apu_id")
//     .eq("presupuesto_id", presupuestoId)
...
//   // mapa id-viejo -> id-nuevo, para poder reescribir padre_id de los hijos
//   // (los ids de los ítems SÍ cambian en la copia -- son filas nuevas)
//   const idNuevoDe = new Map<string, string>()
//   for (const item of itemsActuales ?? []) {
//     idNuevoDe.set(item.id, crypto.randomUUID())
//   }

//   // copiar el APU de cada ítem que tenga uno, para que la versión nueva
//   // no comparta apu_id con la vieja (mismo patrón de "copiar, no
//   // compartir" que ya usa copiarApuParaItem).
//   //
//   // Antes esto hacía 3 queries POR CADA APU distinto (traer, insertar el
//   // apu nuevo, insertar sus líneas) dentro de un for -- con 200 ítems
//   // con APU propio, son hasta 600 idas y vueltas a la base, una tras
//   // otra, para crear una sola versión nueva. Ahora es 1 select + 1
//   // insert + 1 insert, sin importar cuántos APU distintos haya.
...
//     const { data: apusOrigen, error: errorApusOrigen } = await supabase
//       .from("apu")
//       .select("id, codigo, descripcion, item_apu(insumo_id, cantidad, rendimiento, tipo)")
//       .in("id", apuIdsUnicos)

//     if (errorApusOrigen) {
//       throw new Error(errorApusOrigen.message)
//     }

//     // los ids nuevos se generan acá mismo (no se dejan al insert) para
//     // no depender de que Postgres devuelva las filas en el mismo orden
//     // en que se mandaron -- así el mapeo viejo->nuevo es inequívoco.
...
// // Carga los ítems de UNA VERSIÓN ESPECÍFICA (para ver una versión
// // anterior) -- de solo lectura, no importa si es la versión actual o no.
// // A diferencia de cargarItemsDePresupuesto, esto no asume "dame la
// // versión viva", así que sirve tanto para navegar el historial como
// // -- internamente -- para cargar la versión actual también.
...
//     .from("presupuesto_items")
//     .select(
//       "id, padre_id, nivel, codigo, descripcion, unidad, cantidad, valor_unitario, valor_total, apu_id"
//     )
...
// // Actualizar cantidad item ya guardado y re calcula el precio total
...
// // ---------------------------------------------------------------------------
// // Pattern matching por similitud (previene agregar duplicados por error
// // de tipeo -- ej. "TUBERIA PVC 2 PULG" vs "TUBERÍA PVC 2 PULG."). También
// // respeta el filtro de categoría cuando se manda.
// // ---------------------------------------------------------------------------
...
// // ---------------------------------------------------------------------------
// // Panel de aprobación (/admin-insumos)
// // ---------------------------------------------------------------------------
//  // Cambios en app/(app)/presupuestos/actions.ts -- solo el tipo
// // SolicitudInsumo y la función listarSolicitudesInsumos. El resto de
// // actions.ts no cambia.
...
//   createdAt: string
//   // NUEVO -- trazabilidad de la resolución (aprobado/rechazado)
...
//       codigo_maestro_asignado, resuelto_at, resuelto_por,
...
//     // pendientes: más antigua primero (orden de llegada, como antes).
//     // aprobadas/rechazadas: más reciente primero -- es lo que se
//     // quiere ver al auditar "qué se resolvió últimamente".
...
//   // "perfiles" no tiene FK directa desde solicitado_por NI desde
//   // resuelto_por (ambas columnas apuntan a auth.users, perfiles.id
//   // también apunta a auth.users -- son FKs paralelas, no una cadena) --
//   // se cruzan ambas a mano con un solo select a perfiles, mismo patrón
//   // que ya usaba esta función para solicitado_por.
//   const idsPersonas = [
//     ...new Set(
//       filas
...
//   }))
// }
// // Aprobar: crea el insumo real en maestro_insumos con el precio que
// // ingresa quien aprueba, marca la solicitud como aprobada, y -- si la
// // solicitud venía de un ítem del presupuesto -- lo agrega automáticamente
// // al APU de ese ítem (cantidad 1, rendimiento 1, para que quede
// // enlazado; el ingeniero puede ajustar cantidad/rendimiento después
// // desde el editor de APU como con cualquier otra línea).
// export type AprobarSolicitudInput = {
//   solicitudId: string
//   vrUnitario: number
//   // por si quien aprueba quiere corregir tipo/unidad antes de crear el
//   // insumo (ej. el ingeniero escribió algo ambiguo)
//   tipo?: string
//   uM?: string | null
//   agrupacion?: string | null
// }
...
//   // enlazar automáticamente al APU del ítem que originó la solicitud, si
//   // tiene uno -- si el ítem no tiene apu_id todavía (nunca se abrió el
//   // editor de APU), no hay a dónde agregarlo: se deja solo creado en el
//   // maestro, disponible para agregarse a mano.
//   if (solicitud.presupuesto_item_id) {
...
//   items: ItemApu[]
//   // en cuántos presupuesto_items (de cualquier proyecto) está usado
//   // este mismo apu_id -- para avisar que editarlo afecta a todos.
//   usos: number
// }
...
//     usos,
//     items: (fila.item_apu ?? []).map((it: any) => ({
//       id: it.id,
//       insumoId: it.insumo_id,
//       codigo: it.maestro_insumos?.codigo,
//       descripcion: it.maestro_insumos?.descripcion,
//       uM: it.maestro_insumos?.u_m ?? null,
//       vrUnitario: it.maestro_insumos?.vr_unitario ?? null,
//       cantidad: Number(it.cantidad),
//       rendimiento: Number(it.rendimiento ?? 1),
//       tipo: it.maestro_insumos?.tipo ?? it.tipo ?? null,
//     })),
//   }
// }

// const SELECT_APU =
//   "id, codigo, descripcion, item_apu(id, insumo_id, cantidad, rendimiento, tipo, maestro_insumos(codigo, descripcion, u_m, tipo, vr_unitario))"
...
//     const lineas: LineaApuExport[] = (apu.item_apu ?? []).map((item: any) => {
//       const insumo = item.maestro_insumos
//       const cantidad = Number(item.cantidad ?? 0)
//       const rendimiento = Number(item.rendimiento ?? 1)
//       const valorUnitario = insumo?.vr_unitario != null ? Number(insumo.vr_unitario) : null
...
export async function aprobarSolicitudManoObra(
...
): Promise<{ id: string; categoria: string; grupo: string | null; valorUnitario: number }> {
  await requerirScope("admin_mano_obra")
  const supabase = await createClient()
  ...
  const { data: categoriaNueva, error: errorCategoria } = await supabase
    ...
    .select("id, categoria, grupo, valor_unitario")
    .single()
