...
// // ---- Equipo: catálogo de maquinaria/herramienta por HORA ----
// // A diferencia de mano de obra, el equipo SÍ se matchea como un insumo
// // normal: por el TEXTO DE LA LÍNEA del Excel (ej. "Retroexcavadora 320"),
// // no por el nombre del ítem -- la descripción de la línea de equipo sí
// // identifica qué máquina es, a diferencia de "Cuadrilla AA-4". Por eso
// // ResolucionEquipo tiene la MISMA forma que ResolucionInsumo (una
// // resolución por descripción única de línea, con auto_match permitido),
// // en vez de la forma de ResolucionManoObra (por ítem, sin auto_match).
// // Antes estas líneas (tipo === "EQUIPO") matcheaban contra
// // maestro_insumos como cualquier material -- se sacaron de ahí para
// // matchear contra equipo_categorias en su lugar.
...
// export type ResolucionEquipo = {
//   descripcionOriginal: string
//   estado: "auto_match" | "requiere_revision"
//   equipoIdAsignado: string | null
//   candidatosSugeridos: CategoriaEquipo[]
//   precioPlaceholder: boolean
// }
...
//   insumoIdAsignado: string | null
//   candidatosSugeridos: CandidatoInsumo[]
//   // Para tipo === "EQUIPO": misma idea que insumoIdAsignado/
//   // candidatosSugeridos de arriba, pero resuelto contra equipo_categorias
//   // (ver matchearEquipoApuImport en actions.ts). Para cualquier otro
//   // tipo de línea quedan en null/[].
//   equipoIdAsignado: string | null
//   candidatosEquipoSugeridos: CategoriaEquipo[]
// }
...
//   unidad: string | null
//   cantidad: number
//   // Para líneas normales (tipo !== "MO"/"EQUIPO"): CandidatoInsumo[]. Para
//   // mano de obra (tipo === "MO"): CategoriaManoObra[]. Para equipo
//   // (tipo === "EQUIPO"): CategoriaEquipo[]. `tipo` es el discriminador --
//   // revísalo antes de usar `candidatos` para saber cuál de las tres
//   // formas trae.
//   candidatos: CandidatoInsumo[] | CategoriaManoObra[] | CategoriaEquipo[]
//   estado: "auto_match" | "pendiente" | "solicitud_pendiente" | "resuelto" | "rechazado"
//   insumoIdAsignado: string | null
//   manoObraCategoriaIdAsignado: string | null
//   equipoCategoriaIdAsignado: string | null
//   itemApuId: string | null
//   // motivo que escribió el admin al rechazar (viene de
//   // solicitudes_insumos.motivo_rechazo via solicitud_id) -- null si esta
//   // línea no está rechazada, o si el admin no escribió nada.
...
//  * `resolucionesManoObra` es opcional -- se reparte por CÓDIGO DE ÍTEM
//  * (no por descripción de línea, ver nota en ResolucionManoObra arriba).
//  * `resolucionesEquipo` es opcional -- se reparte por DESCRIPCIÓN ÚNICA
//  * de línea, igual que `resoluciones` (insumos), no por ítem.
//  */
// export function combinarBloquesConResoluciones(
//   bloques: BloqueApuInput[],
//   resoluciones: ResolucionInsumo[],
...
//   )
//   const manoObraPorCodigoItem = new Map(resolucionesManoObra.map((r) => [r.codigoItem, r]))
//   const equipoPorDescripcion = new Map(
//     resolucionesEquipo.map((r) => [r.descripcionOriginal.trim().toLowerCase(), r])
//   )
...
//     lineas: bloque.lineas.map((linea) => {
//       const resolucion = porDescripcion.get(linea.descripcion.trim().toLowerCase())
//       const resolucionEquipo = equipoPorDescripcion.get(linea.descripcion.trim().toLowerCase())
//       return {
//         ...linea,
//         estado: resolucion?.estado ?? resolucionEquipo?.estado ?? "requiere_revision",
//         insumoIdAsignado: resolucion?.insumoIdAsignado ?? null,
//         candidatosSugeridos: resolucion?.candidatosSugeridos ?? [],
//         equipoIdAsignado: resolucionEquipo?.equipoIdAsignado ?? null,
//         candidatosEquipoSugeridos: resolucionEquipo?.candidatosSugeridos ?? [],
//       }
//     }),
//     resolucionManoObra: manoObraPorCodigoItem.get(bloque.codigoItem) ?? null,
//   }))
...
export type LineaApuExcelInput = {
  ...
  unidad: string | null
  cantidad: number
  // Solo se usa para tipo === "EQUIPO" -- ver LineaInsumoApu en
  // lib/parse-apu-excel.ts. null si la plantilla no trae la columna
  // "Rendimiento" (hoy siempre null en la práctica -- la plantilla base
  // nunca la tuvo).
...
export type FilaRevisionImport = {
  ...
  // Para líneas normales (tipo !== "MO"/"EQUIPO"): CandidatoInsumo[]. Para
  // mano de obra (tipo === "MO"): CategoriaManoObra[]. Para equipo
  // (tipo === "EQUIPO"): CategoriaEquipo[]. Para transporte
  // (tipo === "TRANSPORTE"): siempre [] -- no hay candidatos, se resuelve
  // por Excel (ver FilaExportTransporte/FilaImportTransporte abajo), no
  // por matching. `tipo` es el discriminador -- revísalo antes de usar
  // `candidatos` para saber cuál de las formas trae.
  candidatos: CandidatoInsumo[] | CategoriaManoObra[] | CategoriaEquipo[]
  estado: "auto_match" | "pendiente" | "solicitud_pendiente" | "resuelto" | "rechazado"
  insumoIdAsignado: string | null
  manoObraCategoriaIdAsignado: string | null
  equipoCategoriaIdAsignado: string | null
  // Precio de transporte ya cargado para esta línea (null si todavía no
  // se ha subido ningún Excel con esta línea resuelta). Se trae directo
  // acá (no requiere una consulta aparte) para que la pestaña "Transporte"
  // del diálogo pueda mostrar "ya tiene precio: $X" sin ida y vuelta extra.
...
// ---- Transporte: roundtrip por Excel, sin catálogo ni similitud ----
// Cada línea de transporte de cada ítem tiene su PROPIO precio -- no se
// reutiliza entre ítems aunque el texto sea idéntico (a diferencia de
// insumos/equipo). El roundtrip usa el id de la fila de
// apu_import_revision como clave exacta (columna oculta en el Excel),
// nunca texto -- así una edición manual del texto en el Excel no rompe
// el matching en silencio.

export type FilaExportTransporte = {
  revisionId: string
  proyectoNombre: string | null
  itemCodigo: string
  itemDescripcion: string
  itemUnidad: string | null
  itemCantidad: number | null
  descripcionTransporte: string
  unidadTransporte: string | null
  cantidadTransporte: number
  // Valor ya cargado, si lo hay -- se incluye en el export para que el
  // mismo Excel sirva tanto para cargar por primera vez como para
  // corregir (el ingeniero ve qué había antes de sobreescribir).
  valorActual: number | null
}

export type FilaImportTransporte = {
  revisionId: string
