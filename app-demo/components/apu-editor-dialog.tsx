...
//   buscarManoObraSimilares,
//   agregarManoObraApu,
//   crearSolicitudManoObra,
...
// // sección. "Materiales" y "Otros" siguen siendo insumo (maestro_insumos,
// // sin cambios); Mano de Obra y Equipo tienen su propio catálogo
// // dedicado; Transporte NO tiene flujo de agregado manual (decisión de
// // negocio explícita, ver handoff -- se resuelve solo por el roundtrip de
// // Excel, nunca desde este diálogo).
// const RECURSO_POR_CATEGORIA: Record<string, "insumo" | "mano_obra" | "equipo" | "transporte"> = {
//   Materiales: "insumo",
//   "Mano de Obra": "mano_obra",
//   "Equipo y Herramienta menor": "equipo",
...
// // A qué categoría (de CATEGORIAS_APU) pertenece una línea del APU.
// // Categoriza por ESTRUCTURA (qué columna FK trae la fila en la base),
// // no por el string `tipo` -- ese string es el token crudo del Excel de
// // origen ("MO", "EQUIPO", "TRANSPORTE") para mano de obra/equipo/
// // transporte, y NUNCA coincide con los valores reales de
// // maestro_insumos.tipo que usa CATEGORIAS_APU ("HONORARIOS - H",
// // "EQUIPO - E", "TRANSPORTE - T", etc.) -- son dos universos de string
// // distintos. Comparar contra `tipo` para estas tres categorías es
// // frágil (dependía de mantener un mapeo paralelo sincronizado a mano);
// // la columna FK poblada es la fuente de verdad real y no puede
// // desincronizarse. Solo para insumos de catálogo (maestro_insumos) el
// // `tipo` real SÍ es el valor correcto a consultar contra CATEGORIAS_APU.
// function categoriaDeItem(it: ItemApu): string {
//   if (it.manoObraCategoriaId) return "Mano de Obra"
//   if (it.equipoCategoriaId) return "Equipo y Herramienta menor"
//   if (it.transportePrecioId) return "Transporte"
//   if (it.esHerramientaMenor) return "Equipo y Herramienta menor"
//   if (!it.tipo) return "Otros"
//   const cat = CATEGORIAS_APU.find((c) => (c.tipos as readonly string[]).includes(it.tipo as string))
//   return cat?.nombre ?? "Otros"
// }

// // El rendimiento solo participa de verdad en el cálculo para insumo y
// // equipo (vrUnitario × cantidad × rendimiento, ver recalcular_valor_apu
// // en la base). Para mano de obra, transporte y herramienta menor el
// // valor de la línea NO se multiplica por rendimiento -- mapearApu ya
// // lo fuerza a 1 al mostrarlo, así que dejarlo editable ahí es engañoso:
// // el cambio se guardaría en la base pero nunca se reflejaría en el
// // total, ni siquiera en la misma pantalla después de recargar.
// function rendimientoEsEditable(it: ItemApu): boolean {
//   return !it.manoObraCategoriaId && !it.transportePrecioId && !it.esHerramientaMenor
// }
...
//                   (it) => categoriaDeItem(it) === cat.nombre
//                 )
...
//   const recurso = RECURSO_POR_CATEGORIA[nombre] ?? "insumo"
...
//           )}
//           {recurso === "transporte" ? (
//             <span className="text-xs text-muted-foreground" title="El transporte no se agrega manualmente acá -- se sube por Excel desde el banner de transporte de la página del presupuesto.">
//               Se administra por Excel
//             </span>
//           ) : (
...
//                   ) : (
//                     <span
//                       title="El rendimiento no aplica a este tipo de línea -- el valor no se multiplica por él (ver recalcular_valor_apu)"
...
//       {abierta && recurso === "mano_obra" && (
//         <div className="border-t bg-muted/10 p-4">
//           <BuscadorManoObraCategoria
//             apuId={apuId}
//             presupuestoItemId={presupuestoItemId}
//             onAgregado={onAgregado}
//           />
...
//       {abierta && recurso === "equipo" && (
//         <div className="border-t bg-muted/10 p-4">
//           <BuscadorEquipoCategoria
//             apuId={apuId}
//             presupuestoItemId={presupuestoItemId}
//             onAgregado={onAgregado}
//           />
...
// // ---------------------------------------------------------------------------
// // Buscador de MANO DE OBRA -- análogo a BuscadorInsumoCategoria, pero
// // contra mano_obra_categorias (buscarManoObraSimilares/agregarManoObraApu).
// // Sin rendimiento: mano de obra no lo usa en el cálculo (ver
// // recalcular_valor_apu / rendimientoEsEditable más arriba), así que ni
// // se pide acá -- pedirlo sería sugerir que sí participa.
// // ---------------------------------------------------------------------------

// function BuscadorManoObraCategoria({
//   apuId,
//   presupuestoItemId,
//   onAgregado,
// }: {
...
// }) {
//   const [busqueda, setBusqueda] = useState("")
//   const [sugerencias, setSugerencias] = useState<CategoriaManoObra[]>([])
//   const [seleccionado, setSeleccionado] = useState<CategoriaManoObra | null>(null)
...
//     }
//     const timeout = setTimeout(() => {
//       buscarManoObraSimilares(busqueda).then(setSugerencias).catch(console.error)
//     }, 300)
//     return () => clearTimeout(timeout)
//   }, [busqueda, seleccionado])

//   function elegirSugerencia(categoria: CategoriaManoObra) {
//     setSeleccionado(categoria)
//     setBusqueda(categoria.categoria)
//     setSugerencias([])
//     setMensaje(null)
//   }
...
//         apuId,
//         manoObraCategoriaId: seleccionado.id,
//         cantidad: Number(cantidad),
//       })
//       onAgregado()
//     } catch (e) {
//       setMensaje(e instanceof Error ? e.message : "No se pudo agregar la categoría de mano de obra.")
//     } finally {
//       setAgregando(false)
//     }
//   }
...
//     setMensaje(null)
//     try {
//       await crearSolicitudManoObra({ descripcion: texto, presupuestoItemId })
//       setSolicitudEnviada(true)
//     } catch (e) {
//       setMensaje(e instanceof Error ? e.message : "No se pudo enviar la solicitud.")
//     } finally {
...
//   const sinPrecio = seleccionado != null && seleccionado.valorUnitario == null
...
//             setSolicitudEnviada(false)
//           }}
//           placeholder="Busca una categoría de mano de obra..."
//           className="h-10"
//         />
//       </div>
//       <DropdownFlotante anchorRef={inputRef} abierto={sugerencias.length > 0 && !seleccionado}>
...
//               <span className="text-xs text-muted-foreground">
//                 {s.grupo ?? "sin grupo"} · {s.unidad} ·{" "}
//                 {s.valorUnitario == null ? (
//                   <span className="text-amber-700">sin precio</span>
//                 ) : (
//                   formatoCOP(s.valorUnitario)
//                 )}
//               </span>
//             </button>
//           ))}
...
//               Esta categoría todavía no tiene precio -- pide que se defina en el catálogo de
//               mano de obra antes de poder agregarla.
//             </p>
//           ) : (
...
//         <div className="space-y-2">
//           <p className="text-xs text-muted-foreground">
//             No hay ninguna categoría de mano de obra parecida a &quot;{busqueda.trim()}&quot;.
//           </p>
//           <Button variant="outline" size="sm" onClick={handleEnviarSolicitud} disabled={enviandoSolicitud}>
//             {enviandoSolicitud ? "Enviando..." : `Solicitar "${busqueda.trim()}" como categoría nueva`}
//           </Button>
//         </div>
//       )}
...
//       {solicitudEnviada && (
//         <p className="text-sm text-emerald-700">
//           ✓ Solicitud enviada -- un admin la revisa en /admin-mano-obra y le asigna grupo,
//           unidad y valor. Cuando se apruebe, si este APU ya está armado, la categoría se
//           agrega sola con cantidad 1 (puedes ajustarla después).
//         </p>
//       )}
...
// // ---------------------------------------------------------------------------
// // Buscador de EQUIPO -- análogo a BuscadorInsumoCategoria, contra
// // equipo_categorias (buscarEquipoSimilares/agregarEquipoApu). A
// // diferencia de mano de obra, equipo SÍ multiplica por cantidad ×
// // rendimiento (fórmula marcada "revisar después" en el handoff), así
// // que el formulario sí pide rendimiento -- igual que insumos.
// // ---------------------------------------------------------------------------

// function BuscadorEquipoCategoria({
//   apuId,
//   presupuestoItemId,
//   onAgregado,
// }: {
...
// }) {
//   const [busqueda, setBusqueda] = useState("")
//   const [sugerencias, setSugerencias] = useState<CategoriaEquipo[]>([])
//   const [seleccionado, setSeleccionado] = useState<CategoriaEquipo | null>(null)
//   const [cantidad, setCantidad] = useState("")
//   const [rendimiento, setRendimiento] = useState("1")
//   const [agregando, setAgregando] = useState(false)
//   const [enviandoSolicitud, setEnviandoSolicitud] = useState(false)
...
//     }
//     const timeout = setTimeout(() => {
//       buscarEquipoSimilares(busqueda).then(setSugerencias).catch(console.error)
//     }, 300)
//     return () => clearTimeout(timeout)
//   }, [busqueda, seleccionado])

//   function elegirSugerencia(categoria: CategoriaEquipo) {
//     setSeleccionado(categoria)
//     setBusqueda(categoria.categoria)
//     setSugerencias([])
//     setMensaje(null)
...
//         apuId,
//         equipoCategoriaId: seleccionado.id,
//         cantidad: Number(cantidad),
//         rendimiento: rendimiento ? Number(rendimiento) : 1,
//       })
//       onAgregado()
//     } catch (e) {
//       setMensaje(e instanceof Error ? e.message : "No se pudo agregar el equipo.")
//     } finally {
//       setAgregando(false)
//     }
//   }
...
//     setMensaje(null)
//     try {
//       await crearSolicitudEquipo({ descripcion: texto, presupuestoItemId })
//       setSolicitudEnviada(true)
//     } catch (e) {
//       setMensaje(e instanceof Error ? e.message : "No se pudo enviar la solicitud.")
//     } finally {
...
//             setSolicitudEnviada(false)
//           }}
//           placeholder="Busca un equipo/maquinaria..."
//           className="h-10"
//         />
//       </div>
//       <DropdownFlotante anchorRef={inputRef} abierto={sugerencias.length > 0 && !seleccionado}>
...
//           <div className="w-32 space-y-1.5">
//             <label className="text-xs text-muted-foreground" title="Multiplicador libre, default 1">
//               Rendimiento
//             </label>
//             <Input
//               type="number"
...
//           {sinPrecio ? (
//             <p className="text-xs text-amber-700">
//               Este equipo todavía no tiene precio -- pide que se defina en el catálogo de
//               equipo antes de poder agregarlo.
//             </p>
//           ) : (
//             <Button onClick={handleAgregar} disabled={agregando}>
//               {agregando ? "Agregando..." : "Agregar"}
...
//           </p>
//           <Button variant="outline" size="sm" onClick={handleEnviarSolicitud} disabled={enviandoSolicitud}>
//             {enviandoSolicitud ? "Enviando..." : `Solicitar "${busqueda.trim()}" como equipo nuevo`}
//           </Button>
//         </div>
//       )}
...
//       {solicitudEnviada && (
//         <p className="text-sm text-emerald-700">
//           ✓ Solicitud enviada -- un admin la revisa en /admin-mano-obra (selector "Equipo") y
//           le asigna grupo, unidad y valor. Cuando se apruebe, si este APU ya está armado, el
//           equipo se agrega solo con cantidad 1 (puedes ajustarla después).
//         </p>
//       )}
...
// Normaliza para comparar sin importar acentos/mayúsculas -- "instalacion"
// debe encontrar "Instalación". Suficiente para un filtro local sobre
// ~150 filas; no hace falta el motor de similitud completo acá.
...
function BuscadorManoObraCategoria({
  ...
  const [busqueda, setBusqueda] = useState("")
  // Catálogo completo, traído UNA vez al abrir esta sección (no en cada
  // tecla) -- ver comentario de listarManoObraCategoriasCompleto en
  // actions.ts. null mientras carga, [] si falló o está vacío.
  const [catalogo, setCatalogo] = useState<CategoriaManoObra[] | null>(null)
  const [seleccionado, setSeleccionado] = useState<CategoriaManoObra | null>(null)
  const [cantidad, setCantidad] = useState("1")
  const [agregando, setAgregando] = useState(false)
  const [enviandoSolicitud, setEnviandoSolicitud] = useState(false)
  ...
  // Filtro 100% local, sin red -- de ahí la ganancia de velocidad vs.
  // el roundtrip al servidor por cada pausa al escribir que había antes.
  ...
  return (
    <div className="space-y-3">
      ...
      {catalogo === null && <p className="text-xs text-muted-foreground">Cargando catálogo...</p>}
      <DropdownFlotante anchorRef={inputRef} abierto={sugerencias.length > 0 && !seleccionado}>
        <div className="max-h-72 w-full overflow-auto rounded-lg border bg-background shadow-lg">
          {sugerencias.map((s) => (
            <button
      ...
      {!seleccionado && busqueda.trim().length >= 2 && catalogo !== null && sugerencias.length === 0 && !solicitudEnviada && (
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            No hay ninguna categoría de mano de obra parecida a &quot;{busqueda.trim()}&quot;.
          </p>
